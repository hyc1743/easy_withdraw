...
- [ ] （已取消）支持 HTTPS 启动（证书/私钥配置）
- [ ] （已取消）HTTPS 模式下输出明确启动日志与访问说明
- [ ] （已取消）保留 HTTP 回退模式（无证书时）

## Phase 13: SQLite 持久化改造

- [x] 引入 SQLite 存储层（`server/db.ts`）
- [x] 将应用配置从 JSON 文件迁移到 SQLite（含自动迁移）
- [x] 将提现历史从 JSON 文件迁移到 SQLite（含自动迁移）
- [x] 定时任务状态与日志持久化到 SQLite
- [x] 进程重启后恢复运行中的定时任务

## Phase 14: 安全与可靠性优化

- [x] `POST /api/auth/unlock` 增加防暴力破解限流
- [x] Gate API 请求增加超时控制
- [x] Gate API 请求增加重试策略（网络错误/429/5xx）
- [x] 提现执行增加幂等键策略（单次与定时任务轮次）
- [x] 明确定义“锁定后定时任务继续执行”的行为并在文档说明

## Phase 15: 文档与验收

- [x] 更新 README（HTTPS/SQLite/安全策略/API 变化）
- [ ] 回归检查：定时任务在刷新页面与服务重启后的连续性
- [ ] 回归检查：锁定后定时任务继续执行
